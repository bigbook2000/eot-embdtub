...
// DMA信息
static EOTDMAInfo s_DMAInfoRecv;
// DMA输入数据缓存
D_STATIC_BUFFER_DECLARE(s_DMABufferRecv, DEBUG_BUFFER_SIZE)
...
{
	D_STATIC_BUFFER_INIT(s_DMABufferRecv, DEBUG_BUFFER_SIZE);
...
//	LL_USART_EnableIT_RXNE(UART_DEBUG);
	...
	// 配置DMA
	EOB_DMA_Recv_Init(&s_DMAInfoRecv,
...
{
	int nLen = EOB_DMA_Recv(&s_DMAInfoRecv);
	if (nLen < 0)
	{
	...
	{
		s_DMABufferRecv.buffer[s_DMABufferRecv.length] = '\0';
		printf("Debug DMA 接收数据 [%d]%s\n",
				s_DMABufferRecv.length, s_DMABufferRecv.buffer);
	}
